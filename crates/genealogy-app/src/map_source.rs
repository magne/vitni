...
//! here, in the app layer (ADR 0008): `genealogy-ui-dioxus` never fetches, it only renders whatever
...
/// serves. Matches the map's own camera ceiling (`genealogy_ui::ZOOM_RANGE.1`) today — kept as a
/// literal here since `genealogy-app` sits below `genealogy-ui` (ADR 0008) and cannot import it.
...
mod tests {
    ...
    fn a_configured_api_key_env_substitutes_the_key_placeholder() {
        unsafe { std::env::set_var("GENEALOGY_TEST_MAP_KEY_SUBSTITUTE", "secret-123") };
        let resolved = resolve_style_url(
            "https://example.test/style.json?key={key}",
            Some("GENEALOGY_TEST_MAP_KEY_SUBSTITUTE"),
        )
        .expect("resolve");
        assert_eq!(resolved, "https://example.test/style.json?key=secret-123");
        unsafe { std::env::remove_var("GENEALOGY_TEST_MAP_KEY_SUBSTITUTE") };
    }

    #[test]
    fn an_unset_api_key_env_is_a_named_config_error() {
        unsafe { std::env::remove_var("GENEALOGY_TEST_MAP_KEY_UNSET") };
        let error = resolve_style_url(
            "https://example.test/style.json?key={key}",
            Some("GENEALOGY_TEST_MAP_KEY_UNSET"),
        )
        ...
        assert!(
            message.contains("GENEALOGY_TEST_MAP_KEY_UNSET"),
    ...
    fn an_api_key_env_with_no_placeholder_in_the_url_is_a_named_config_error() {
        unsafe { std::env::set_var("GENEALOGY_TEST_MAP_KEY_NO_PLACEHOLDER", "secret-123") };
        let error = resolve_style_url(
            "https://example.test/style.json",
            Some("GENEALOGY_TEST_MAP_KEY_NO_PLACEHOLDER"),
        )
        ...
        assert!(
            message.contains("GENEALOGY_TEST_MAP_KEY_NO_PLACEHOLDER"),
            "names the offending env var: {message}"
        );
        unsafe { std::env::remove_var("GENEALOGY_TEST_MAP_KEY_NO_PLACEHOLDER") };
    }
    ...
    async fn a_maplibre_provider_naming_an_unset_env_var_fails_before_any_network_call() {
        let provider = MapProvider::MaplibreStyle {
            ...
            api_key_env: Some("GENEALOGY_TEST_MAP_KEY_ALSO_UNSET".to_owned()),
        };
