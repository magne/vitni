...
fn sorted_participations(participations: &[vitni_app::ParticipationRef]) -> Vec<&vitni_app::ParticipationRef> {
    let mut ordered: Vec<&vitni_app::ParticipationRef> = participations.iter().collect();
...
/// the joined [`GenealogicalDate`](vitni_app::GenealogicalDate) sort key with undated rows last.
///
