...
pub struct NameVm {
    ...
    pub name_type: vitni_app::NameType,
...
pub struct FactVm {
    ...
    pub fact_type: vitni_app::FactType,
...
pub struct EventRefVm {
    ...
    pub role: vitni_app::ParticipantRole,
...
pub struct AssociationVm {
    ...
    pub role: vitni_app::AssociationRole,
...
pub struct MediaRefVm {
    ...
    pub crop: Option<vitni_app::Rect>,
...
impl MediaRefVm {
    /// Builds a [`MediaRefVm`] from an app [`MediaRefSummary`](vitni_app::MediaRefSummary).
    #[must_use]
    pub fn from_ref(reference: &vitni_app::MediaRefSummary) -> Self {
        Self {
            human_id: reference.human_id.clone(),
            assertion_id: reference.assertion_id.clone(),
...
/// [`media_root_relative`](vitni_app::media_root_relative), whether or not the stored path already
/// carries it, and the remainder is percent-encoded per segment by
/// [`media_url_path`](vitni_app::media_url_path) so a stored `æøå`, a space or a `#` still names its
...
pub fn media_asset_src(location: Option<&str>) -> Option<String> {
    ...
    let rel = vitni_app::media_root_relative(location)?;
    Some(format!("/{}/{}", vitni_app::MEDIA_DIR, vitni_app::media_url_path(rel)))
}
...
/// ([`mime_for_path`](vitni_app::mime_for_path)) — nothing writes an inferred MIME back into the
/// record, so this is a display rule, not a stored one.
#[must_use]
pub fn media_is_image(mime: Option<&str>, location: Option<&str>) -> bool {
    let mime = match mime {
        Some(mime) => mime,
        None => match location.and_then(vitni_app::mime_for_path) {
...
/// Builds a [`CitationRefVm`] from an app [`CitationRef`](vitni_app::CitationRef) — the joined
/// citation row used by the Event/Place Citations tabs (source label, page, surety, evidence axes).
#[must_use]
pub fn citation_ref_from_ref(reference: &vitni_app::CitationRef, loc: &Localizer) -> CitationRefVm {
