...
impl NoteDetail {
    /// Builds a detail view from a [`NoteSummary`](vitni_app::NoteSummary), localizing the type
    /// label. The History tab starts empty and is filled by the dispatcher.
    #[must_use]
    pub fn from_summary(summary: &vitni_app::NoteSummary, loc: &Localizer) -> Self {
        Self {
...
/// Builds a generic list row from a [`NoteSummary`](vitni_app::NoteSummary): a title from the
/// text, a `type · language · N references` subtitle, and a 🗒 avatar.
#[must_use]
pub fn note_row(summary: &vitni_app::NoteSummary, loc: &Localizer) -> RowVm {
