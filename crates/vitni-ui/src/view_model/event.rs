...
impl EventDetail {
    /// Builds a detail view from an [`EventSummary`](vitni_app::EventSummary), localizing labels,
    /// dates, and confidence. The History tab starts empty and is filled by the dispatcher.
    #[must_use]
    pub fn from_summary(summary: &vitni_app::EventSummary, loc: &Localizer) -> Self {
...
/// Builds a generic list row from an [`EventSummary`](vitni_app::EventSummary): the type label,
/// a `date · place` subtitle, and a per-type avatar.
#[must_use]
pub fn event_row(summary: &vitni_app::EventSummary, loc: &Localizer) -> RowVm {
    let place = summary
        .place
        .as_ref()
...
mod event_detail_tests {
    ...
    use vitni_app::{Confidence, EventSummary, EventType, ParticipantRef, ParticipantRole};
