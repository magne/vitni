...
//! `vitni-app`/`vitni-core` free of UI text.
...
impl Localizer {
    ...
    pub fn field_label(&self, id: &str) -> String {
        match id {
            ...
            "typical-confidence" => fl!(self.loader, "field-typical-confidence"),
            ...
            "fax" => fl!(self.loader, "field-fax"),
            "www" => fl!(self.loader, "field-www"),
    ...
    #[must_use]
    pub fn event_type_label(&self, event_type: &vitni_app::EventType) -> String {
    ...
    #[must_use]
    pub fn place_type_label(&self, place_type: &vitni_app::PlaceType) -> String {
    ...
    /// ADR 0026 §2–§3).
    #[must_use]
    pub fn succession_kind_label(&self, kind: vitni_app::SuccessionKind) -> String {
    ...
    #[must_use]
    pub fn plugin_trust_label(&self, trust: vitni_app::PluginTrust) -> String {
        match trust {
            vitni_app::PluginTrust::Sanctioned => fl!(self.loader, "plugin-trust-sanctioned"),
            vitni_app::PluginTrust::UserTrusted => fl!(self.loader, "plugin-trust-user-trusted"),
            vitni_app::PluginTrust::Untrusted => fl!(self.loader, "plugin-trust-untrusted"),
        }
    }
    ...
    pub fn panel_title(&self, id: &str) -> String {
        match id {
            ...
            "edit-address" => fl!(self.loader, "panel-edit-address"),
    ...
    fn event_type_summary(&self, event_type: &str) -> String {
        match event_type {
            ...
            "GeometryAsserted" => fl!(self.loader, "history-geometry-asserted"),
            "SuccessionAsserted" => fl!(self.loader, "history-succession-asserted"),
...
fn plugin_loader(catalogue_dir: &Path, domain: &str, requested: &[LanguageIdentifier]) -> Option<FluentLanguageLoader> {
    ...
    let chain: Vec<LanguageIdentifier> = vitni_i18n::fallback_chain(requested, &fallback)
        .into_iter()
        .filter(|lang| {
            catalogue_dir
                .join(lang.to_string())
...
mod tests {
    ...
    fn agent_name_with_kind_annotates_a_software_agent() {
        ...
        assert_eq!(
            loc.agent_name_with_kind("vitni-import", OperatorKind::Software),
            "vitni-import (software agent)"
        );
        assert_eq!(
    ...
    fn a_workspace_override_wins_over_the_embedded_baseline() {
        ...
        std::fs::write(en_dir.join("vitni-ui.ftl"), "list-empty = OVERRIDDEN\n").expect("write override");
    ...
    fn person_ref(human_id: &str, name: &str) -> vitni_app::PedigreePersonRef {
        vitni_app::PedigreePersonRef {
