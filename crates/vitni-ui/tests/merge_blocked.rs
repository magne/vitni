...
//! `None` (the screen keeps its toast). Built against vitni-app's public surface only, so the
//! presentation layer never names a `vitni-core` type, even in tests (ADR 0008).
