...
//! aggregate's `Services` type can name it; the SQLite implementation lives in `vitni-db`.
...
///
/// Implemented by `vitni-db` over the Person projection; the `cqrs-es` `Services` value for the
