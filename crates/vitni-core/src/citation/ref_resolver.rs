...
//!
//! The trait lives in `vitni-core` so the aggregate's `Services` type can name it; the concrete
//! implementation (a SQLite projection query) lives in `vitni-db`.
...
///
/// Implemented by `vitni-db` over the Source projection; the `cqrs-es` `Services` value for the
