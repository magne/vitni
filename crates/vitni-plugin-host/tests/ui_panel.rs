...
//! (`vitni-ui-dioxus`) asserts full `vitni_ui::Form` conformance and rendering.
//!
...
use vitni_plugin_host::{Capability, Grants, ResourceBudget};
...
async fn handle_action_with_commands_grant_creates_a_note() {
    ...
    let counts = vitni_app::workspace_counts(&workspace).await.expect("counts");
