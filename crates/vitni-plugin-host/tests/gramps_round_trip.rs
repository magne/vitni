...
fn software_session() -> Session {
    Session::software("vitni-gramps-import", "0.1.0")
}
...
async fn assert_breadth(workspace: &Workspace) {
    ...
    assert_eq!(john.sex, Some(vitni_app::Sex::Male), "gender M imported");
    assert_eq!(
        john.citations.len(),
        1,
    ...
    assert_eq!(
        relationships,
        vec![
            vitni_app::ChildParentRelationship::Adopted,
            vitni_app::ChildParentRelationship::Birth,
        ],
