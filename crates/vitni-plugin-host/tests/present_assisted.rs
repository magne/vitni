...
//! and the session state machine are unit-tested in `vitni-ui`.
//!
...
fn assisted_invocation(workspace: Workspace, grants: Grants) -> vitni_plugin_host::Invocation {
    vitni_plugin_host::Invocation {
        workspace,
...
async fn run_assisted_import_drives_a_session_to_its_summary() {
    ...
    let (returned, _ws) = common::host()
        .run_assisted_import(
            ...
            request,
            Box::new(presenter),
            |_update| vitni_plugin_host::ProgressControl::Proceed,
        )
        .await
