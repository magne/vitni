...
//! `vitni-app` use-cases through a [`Session`] whose operator is `AgentKind::Software`, so every
...
use crate::bindings::imports::vitni::host_api::{
...
impl commands::Host for HostState {
    ...
    async fn assert_fact(
        ...
        vitni_app::assert_fact(&self.workspace, &self.session, &person, new, self.mutation_meta())
            .await
            .map_err(|error| to_capability_error(&error))
    }
...
fn from_fact(fact: &vitni_core::fact::Fact) -> types::Fact {
    types::Fact {
...
impl query::Host for HostState {
    async fn list_persons(&mut self) -> Result<Vec<types::PersonDto>, types::CapabilityError> {
        ...
        let persons = vitni_app::list_persons(&self.workspace)
            .await
            .map_err(|error| to_capability_error(&error))?;
        Ok(persons
            .into_iter()
    ...
    async fn list_families(&mut self) -> Result<Vec<types::FamilyDto>, types::CapabilityError> {
        ...
        let families = vitni_app::list_families(&self.workspace)
            .await
            .map_err(|error| to_capability_error(&error))?;
        Ok(families
            .into_iter()
    ...
    async fn list_events(&mut self) -> Result<Vec<types::EventDto>, types::CapabilityError> {
        ...
        let events = vitni_app::list_events(&self.workspace)
            .await
            .map_err(|error| to_capability_error(&error))?;
        Ok(events
            .into_iter()
    ...
    async fn list_sources(&mut self) -> Result<Vec<types::SourceDto>, types::CapabilityError> {
        ...
        let sources = vitni_app::list_sources(&self.workspace)
            .await
            .map_err(|error| to_capability_error(&error))?;
        Ok(sources
            .into_iter()
    ...
    async fn list_citations(&mut self) -> Result<Vec<types::CitationDto>, types::CapabilityError> {
        ...
        let citations = vitni_app::list_citations(&self.workspace)
            .await
            .map_err(|error| to_capability_error(&error))?;
        Ok(citations
            .into_iter()
    ...
    async fn list_media(&mut self) -> Result<Vec<types::MediaDto>, types::CapabilityError> {
        ...
        let media = vitni_app::list_media(&self.workspace)
            .await
            .map_err(|error| to_capability_error(&error))?;
        Ok(media
            .into_iter()
    ...
    async fn list_notes(&mut self) -> Result<Vec<types::NoteDto>, types::CapabilityError> {
        ...
        let notes = vitni_app::list_notes(&self.workspace)
            .await
            .map_err(|error| to_capability_error(&error))?;
        Ok(notes
            .into_iter()
    ...
    async fn list_repositories(&mut self) -> Result<Vec<types::RepositoryDto>, types::CapabilityError> {
        ...
        let repositories = vitni_app::list_repositories(&self.workspace)
            .await
            .map_err(|error| to_capability_error(&error))?;
        Ok(repositories
            .into_iter()
    ...
    async fn list_tags(&mut self) -> Result<Vec<types::TagDto>, types::CapabilityError> {
        ...
        let tags = vitni_app::list_tags(&self.workspace)
            .await
            .map_err(|error| to_capability_error(&error))?;
        Ok(tags
            .into_iter()
    ...
    async fn list_places(&mut self) -> Result<Vec<types::PlaceDto>, types::CapabilityError> {
        ...
        let places = vitni_app::list_places(&self.workspace)
            .await
            .map_err(|error| to_capability_error(&error))?;
        Ok(places
            .into_iter()
...
fn to_confidence(confidence: types::Confidence) -> vitni_app::Confidence {
    match confidence {
        ...
        types::Confidence::Normal => vitni_app::Confidence::Normal,
        types::Confidence::High => vitni_app::Confidence::High,
        types::Confidence::VeryHigh => vitni_app::Confidence::VeryHigh,
    }
}

/// Maps the domain [`Confidence`](vitni_app::Confidence) back onto the WIT `confidence` enum.
fn from_confidence(confidence: vitni_app::Confidence) -> types::Confidence {
    match confidence {
        vitni_app::Confidence::VeryLow => types::Confidence::VeryLow,
        vitni_app::Confidence::Low => types::Confidence::Low,
