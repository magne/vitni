...
/// ([`mime_for_path`](vitni_core::media_path::mime_for_path)), else `application/octet-stream`.
fn mime_of(content_type: Option<&str>, rel: &Path) -> String {
    ...
    rel.to_str()
        .and_then(vitni_core::media_path::mime_for_path)
        .unwrap_or("application/octet-stream")
        .to_owned()
}

/// The workspace-relative path string (`media/…`, forward slashes) for a resolved target — the stored
/// form of a [`MediaPath::File`](vitni_core::media_path::MediaPath::File), built by the one owner of
...
fn workspace_relative(media_root: &Path, target: &Path) -> String {
    ...
    if parts.is_empty() {
        return vitni_core::media_path::MEDIA_DIR.to_owned();
    }
    vitni_core::media_path::workspace_media_path(&parts.join("/"))
}
