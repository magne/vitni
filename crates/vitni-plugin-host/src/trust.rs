...
const PROJECT_PUBLIC_KEY_ENV: Option<&str> = option_env!("VITNI_PROJECT_PUBLIC_KEY");
...
impl TrustRoots {
    ...
    /// `VITNI_PROJECT_PUBLIC_KEY` (64-hex); when it is unset the sanctioned set is empty (the
...
///
/// The pins come from `vitni-app`'s client-scope trust store (hex-decoded there); this crate owns
