...
//! rather than over [`vitni_core::place::PlaceView`] directly, so it is unit-testable with a
//! trivial in-memory resolver — no store, no folded event history required.

use std::collections::HashSet;

use vitni_core::ids::{AssertionId, PlaceId};
...
pub(crate) struct HierarchyHop {
    ...
    pub date: Option<vitni_core::date::GenealogicalDate>,
