...
use vitni_core::ids::{AssertionId, HumanId, ResearchNoteId, TagId};
...
/// [`vitni_core::research_note::ResearchNoteError::SubjectRequired`] if `new.subjects` is empty
...
/// [`vitni_core::research_note::ResearchNoteError::UnknownSubject`] on the rare race where the
...
/// [`AppError::ResearchNoteNotFound`] if no such research note exists, a `*NotFound` error if
/// `subject`'s `human_id` is unknown, [`AppError::ResearchNoteDomain`] with
/// [`vitni_core::research_note::ResearchNoteError::SubjectRequired`] if `subject` is the note's
...
/// field on the subject aggregate), via the `vitni-db` JSON reverse index over `subjects`.
///
/// # Errors
///
/// A store/read-model error.
