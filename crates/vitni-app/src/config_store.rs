...
//! order **plain env < configured `ui_language` < `VITNI_LANGUAGE`**. It is pure (takes the
...
const LANGUAGE_ENV: &str = "VITNI_LANGUAGE";
...
/// Reads and parses the `VITNI_LANGUAGE` override (ADR 0015 §4); `None` when unset, empty, or not
/// a valid BCP-47 tag.
#[must_use]
pub fn vitni_language_env() -> Option<LanguageIdentifier> {
...
/// The language request for a real startup: overlays [`vitni_language_env`] on top of the
...
pub fn requested_languages_for(
    ...
    resolve_requested_languages(config_ui_language, plain_env, vitni_language_env().as_ref())
}
...
mod tests {
    ...
    fn file_store_round_trips_workspace_functionality_scope() {
        ...
        assert_eq!(reloaded.database_url, "sqlite://vitni.sqlite3");
    ...
    fn file_store_round_trips_surety_label_overrides_in_the_workspace_scope() {
        ...
        assert_eq!(functionality.database_url, "sqlite://vitni.sqlite3");
...
"#,
        ...
        std::fs::write(
            ...
            r#"
database_url = "sqlite://vitni.sqlite3"
