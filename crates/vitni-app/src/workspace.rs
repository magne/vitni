...
pub struct PluginPreferences {
    ...
    /// approved-capability set of [`Capability::interface_name`](vitni_core) short names (`log`,
...
impl Workspace {
    ...
    /// `vitni rebuild` command. Engine-neutral — works for whichever backend this workspace uses.
    ///
    /// # Errors
    ///
...
pub(crate) fn read_manifest(dir: &Path) -> Result<WorkspaceManifest, AppError> {
    ...
    let text = std::fs::read_to_string(&path)
        .map_err(|e| AppError::Workspace(format!("reading {} (run `vitni init`?): {e}", path.display())))?;
...
mod tests {
    ...
    fn init_creates_the_tree_and_leaves_id_formats_unset() {
        ...
        assert_eq!(manifest.database_url, "sqlite://vitni.sqlite3");
        assert_eq!(
    ...
    async fn open_resolves_the_surety_label_overrides_onto_the_workspace() {
        ...
        assert_eq!(
            workspace
                .surety_labels()
                .label_for(vitni_core::provenance::Confidence::VeryLow),
...
"#,
    ...
    fn a_manifest_without_a_ui_table_parses_to_defaults() {
        ...
        let ws = dir.path().join("ws");
        std::fs::create_dir_all(&ws).expect("dir");
        std::fs::write(ws.join("workspace.toml"), "database_url = \"sqlite://vitni.sqlite3\"\n").expect("write");
    ...
    fn relative_sqlite_url_resolves_against_the_dir() {
        let resolved = resolve_database_url(Path::new("/data/ws"), "sqlite://vitni.sqlite3");
        assert_eq!(resolved, "sqlite:///data/ws/vitni.sqlite3");
    }
