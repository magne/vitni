...
macro_rules! for_each_aggregate {
    ($callback:ident) => {
        $callback! {
            (person, "person", vitni_core::ids::PersonId, new_person_id, vitni_core::person::PersonError, Domain, PersonNotFound, "no person with human_id {0:?}"),
            (family, "family", vitni_core::ids::FamilyId, new_family_id, vitni_core::family::FamilyError, FamilyDomain, FamilyNotFound, "no family with human_id {0:?}"),
            (place, "place", vitni_core::ids::PlaceId, new_place_id, vitni_core::place::PlaceError, PlaceDomain, PlaceNotFound, "no place with human_id {0:?}"),
            (source, "source", vitni_core::ids::SourceId, new_source_id, vitni_core::source::SourceError, SourceDomain, SourceNotFound, "no source with human_id {0:?}"),
            (citation, "citation", vitni_core::ids::CitationId, new_citation_id, vitni_core::citation::CitationError, CitationDomain, CitationNotFound, "no citation with human_id {0:?}"),
            (event, "event", vitni_core::ids::EventId, new_event_id, vitni_core::event::EventError, EventDomain, EventNotFound, "no event with human_id {0:?}"),
            (dna_test, "DNA test", vitni_core::ids::DnaTestId, new_dna_test_id, vitni_core::dna_test::DnaTestError, DnaTestDomain, DnaTestNotFound, "no dna test with human_id {0:?}"),
            (dna_match, "DNA match", vitni_core::ids::DnaMatchId, new_dna_match_id, vitni_core::dna_match::DnaMatchError, DnaMatchDomain, DnaMatchNotFound, "no dna match with human_id {0:?}"),
            (repository, "repository", vitni_core::ids::RepositoryId, new_repository_id, vitni_core::repository::RepositoryError, RepositoryDomain, RepositoryNotFound, "no repository with human_id {0:?}"),
            (note, "note", vitni_core::ids::NoteId, new_note_id, vitni_core::note::NoteError, NoteDomain, NoteNotFound, "no note with human_id {0:?}"),
