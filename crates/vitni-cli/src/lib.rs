//! The terminal frontend — a thin layer over `vitni-app` (ADR 0006).
//!
...
//! command execution — lives in `vitni-app`. stdout/stderr are the interface, so the print
...
//!
//! Library + binary (ADR 0035): [`run`] is the whole entry point, so the `vitni-cli` binary and the
//! `vitni` launcher share one implementation. [`run`] owns the tokio runtime internally, which is
//! what lets the launcher's `main` stay sync for `tao`'s main-thread requirement.
//!
//! # Licence
//!
...
#[derive(Parser)]
#[command(name = "vitni", version, about)]
struct Cli {
    /// Workspace name (overrides the default and `VITNI_WORKSPACE`).
...
/// position; `Init` is handled in [`run_command`] before the workspace opens.
...
/// Parses the arguments and runs the terminal frontend, returning the process exit code.
///
/// `#[tokio::main]` makes this a **sync** `fn` to its caller — the runtime is built and dropped here,
/// so the `vitni` launcher can call it from a sync `main` and still open a webview on that thread in
/// the other arm (ADR 0035 §3).
///
/// # Panics
///
/// Panics if the tokio runtime cannot be built: a host-resource failure with no working frontend left
...
pub async fn run() -> ExitCode {
    ...
    // The workspace-aware `run_command` future is large (a grown `Localizer` plus per-command
    // inputs); box it so it doesn't sit inline on the entry point's stack (clippy::large_futures).
    Box::pin(run_command(Cli::parse())).await
}
...
fn workspace_from_env() -> Option<String> {
    std::env::var("VITNI_WORKSPACE").ok().filter(|s| !s.is_empty())
}
...
async fn init(
    ...
    let summary = vitni_app::register_workspace(&config_path, &name, Some(&path), database_url.as_deref()).await?;
