//! Localization for the `vitni` CLI (ADR 0003).
//!
...
//! localized strings, keeping `vitni-app`/`vitni-core`/`vitni-db` free of UI text.
...
impl Localizer {
    ...
    /// negotiating the request. Used for `init` and any error before a workspace is open; there is no
    /// configured `ui_language` yet, so only the ambient env and `VITNI_LANGUAGE` apply.
    #[must_use]
    ...
    /// Resolves the language request (plain env < `config_ui_language` < `VITNI_LANGUAGE`) and
    ...
    /// [`Self::confidence`] before falling back to the Fluent-resolved default. A builder method
    /// (rather than a constructor parameter) so every existing call site keeps working unchanged;
    /// only a workspace-scoped localizer need call it once, after the workspace opens.
    #[must_use]
    ...
    fn plugin_trust_label(&self, trust: vitni_plugin_host::TrustTier) -> String {
        match trust {
            vitni_plugin_host::TrustTier::Sanctioned => fl!(self.loader, "plugin-trust-sanctioned"),
            vitni_plugin_host::TrustTier::UserTrusted => fl!(self.loader, "plugin-trust-user-trusted"),
            vitni_plugin_host::TrustTier::Untrusted => fl!(self.loader, "plugin-trust-untrusted"),
        }
    }
    ...
    pub fn plugin_list_line(
        &self,
        id: &str,
        trust: vitni_plugin_host::TrustTier,
        declared: &str,
        granted: &str,
    ) -> String {
        fl!(
...
mod tests {
    ...
    fn family_summary_is_localized() {
        let partner = |human_id: &str| vitni_app::PartnerRef {
            human_id: human_id.to_owned(),
            id: String::new(),
            name: None,
            vitals: None,
            confidence: Some(vitni_app::Confidence::Normal),
            source_count: 0,
            citations: Vec::new(),
    ...
    #[test]
    fn confidence_uses_the_fluent_default_when_no_surety_override_is_set() {
        assert_eq!(localizer("en").confidence(Confidence::Normal), "normal");
    }

    #[test]
    fn confidence_prefers_a_workspace_surety_override() {
        let overrides = vitni_app::SuretyLabelOverrides {
            normal: Some(vitni_app::SuretyLabelOverride {
                label: "Balanced".to_owned(),
                description: None,
            }),
            ..Default::default()
        };
        let loc = localizer("en").with_surety_overrides(overrides);
        assert_eq!(
            loc.confidence(Confidence::Normal),
            "Balanced",
            "the workspace's own wording wins over the Fluent default"
        );
        assert_eq!(
            loc.confidence(Confidence::High),
            "high",
    ...
    fn a_workspace_override_wins_over_the_embedded_baseline() {
        ...
        std::fs::write(en_dir.join("vitni-cli.ftl"), "created = OVERRIDDEN { $id }\n").expect("write override");
