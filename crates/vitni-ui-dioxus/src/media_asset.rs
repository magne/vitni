...
/// remainder is percent-decoded ([`media_url_decode`](vitni_app::media_url_decode)) and must be a
...
///
/// Every URL reaching here is built by one owner (`vitni_ui`'s `media_asset_src`, over the same
/// `vitni_core::media_path` rules this reads back), so the mapping is literal: a request missing the
...
pub fn resolve_media_path(media_root: &Path, request_path: &str) -> Option<PathBuf> {
    let rel = vitni_app::media_url_decode(request_path.strip_prefix("/media/")?)?;
...
/// [`mime_for_path`](vitni_app::mime_for_path) — the same one that decides whether a record with no
...
pub fn content_type(path: &Path) -> &'static str {
    path.to_str()
        .and_then(vitni_app::mime_for_path)
...
mod tests {
    ...
    fn a_request_without_the_prefix_is_rejected() {
        ...
        // (`vitni_core::media_path`) builds every URL, so the handler can be literal. The old
