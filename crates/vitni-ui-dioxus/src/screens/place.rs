use vitni_app::{MapProvider, PlaceGeometry, PlaceType};
...
fn place_coordinate_fields(
    loc: &Localizer,
    editing: bool,
    mut draft: Signal<vitni_ui::PlaceDraft>,
    seed: Signal<vitni_ui::PlaceDraft>,
    detail: Option<&PlaceDetail>,
) -> Element {
    let current = draft();
    let committed = seed.read().clone();
    ...
    let displayed = (!editing)
        .then(|| detail.and_then(vitni_ui::display_coordinates))
        .flatten();
...
pub(crate) fn PlaceDetailPane(human_id: String) -> Element {
    ...
    let active = use_detail_tab(Category::Places, &human_id);
    ...
    let DetailCommits {
        ...
        on_retract,
        on_retract_confirm,
    } = use_detail_commits::<PlaceCommits, PlaceEditForm>(&state, &human_id, editing);
    ...
    let seed = match &*data.read_unchecked() {
        Some(ScreenData::Loaded(IntentOutcome::PlaceDetail(detail))) => vitni_ui::PlaceDraft::from_detail(detail),
        _ => vitni_ui::PlaceDraft::new(),
    };
    let record = use_record_edit::<vitni_ui::PlaceDraft>(Category::Places, &human_id, &seed);
    ...
    let on_record_save = use_callback(move |(draft, prov): (vitni_ui::PlaceDraft, ProvenanceDraft)| {
    ...
    use_save_on_request(EditKey::saved(Category::Places, &human_id), record, save_now);
...
struct PlaceCallbacks {
    ...
    on_record_save: Callback<(vitni_ui::PlaceDraft, ProvenanceDraft)>,
...
/// The row callbacks a place's tabs dispatch through, grouped so the tab dispatcher stays under the
/// argument limit.
#[derive(Clone, Copy)]
struct PlaceTabCallbacks {
    ...
    /// Arms the untag panel for a tag chip's ×: `(tag_id, tag name)`.
    on_tag_remove: Callback<(String, String)>,
    /// Reloads the record after the Map tab commits a geometry edit.
    on_map_saved: Callback<()>,
    /// The Media tab's viewer state + crop-supersede wiring.
    media_state: MediaTabState,
}
...
fn place_tab_content(
    ...
    record: RecordEditState<vitni_ui::PlaceDraft>,
    callbacks: PlaceTabCallbacks,
) -> Element {
    let loc = state.data_loc();
    let PlaceTabCallbacks {
        on_retract,
        on_edit_open,
        on_undo,
        on_tag_remove,
    ...
    let shared = SharedTabCtx {
        forms: Some(FormTabs {
            ...
            citations: Some(CitationsArm {
                form: PlaceEditForm::Citation,
                rows: &detail.citations,
                show_backs: false,
                on_detach: on_retract,
            }),
            media: Some(MediaArm {
                form: PlaceEditForm::Media,
                rows: &detail.media,
                state: media_state,
                on_detach: on_retract,
            }),
            notes: Some(NotesArm {
                form: PlaceEditForm::Note,
                rows: &detail.notes,
                on_detach: on_retract,
            }),
            tags: Some(TagsArm {
                form: PlaceEditForm::Tag,
                rows: &detail.tags,
    ...
    match tab.id {
        ...
        _ => shared_tab(loc, tab, &shared).unwrap_or_else(|| place_overview(loc, detail, record)),
    }
}
...
pub fn place_overview(loc: &Localizer, detail: &PlaceDetail, record: RecordEditState<vitni_ui::PlaceDraft>) -> Element {
...
fn succession_row(
    loc: &Localizer,
    this_title: &str,
    rel: &vitni_ui::PlaceSuccessionVm,
    is_predecessor: bool,
    on_retract: Callback<(String, String, bool)>,
) -> Element {
...
/// The Succession panel's live state: the picked kind (an index into [`vitni_ui::SUCCESSION_KINDS`]),
...
pub struct SuccessionFormState {
    /// The picked kind's index into [`vitni_ui::SUCCESSION_KINDS`].
...
pub fn place_succession_form_fields(
    ...
    let options: Vec<SelectChoice> = vitni_ui::SUCCESSION_KINDS
        .iter()
        .enumerate()
...
fn succession_edit(
    ...
    Some(PlaceEdit::AssertSuccession {
        ...
        kind: *vitni_ui::SUCCESSION_KINDS.get(kind_index)?,
...
mod succession_form_tests {
    ...
    use vitni_ui::{DateDraft, DateModifierKind, PickerSelection, PlaceEdit};
...
mod map_editor_tests {
    ...
    use vitni_ui::{Localizer, MarkerShapeVm};
