...
use vitni_ui::{ResearchNoteDetail, ResearchNoteEdit, SubjectVm, research_note_tabs};
...
/// The create-mode research-note record: an uncommitted [`ResearchNoteDraft`](vitni_ui::ResearchNoteDraft)
...
pub fn ResearchNoteCreateRecord(draft_id: DraftId) -> Element {
    ...
    let record = use_record_create::<vitni_ui::ResearchNoteDraft>(Category::ResearchNotes, draft_id);
...
pub fn research_note_record_fields(loc: &Localizer, record: RecordEditState<vitni_ui::ResearchNoteDraft>) -> Element {
...
pub fn research_note_draft_subjects(loc: &Localizer, record: RecordEditState<vitni_ui::ResearchNoteDraft>) -> Element {
    let mut draft = record.draft;
...
pub(crate) fn ResearchNoteDetailPane(human_id: String) -> Element {
    ...
    let seed = match &*data.read_unchecked() {
        Some(ScreenData::Loaded(IntentOutcome::ResearchNoteDetail(detail))) => {
            vitni_ui::ResearchNoteDraft::from_detail(detail)
        }
        _ => vitni_ui::ResearchNoteDraft::new(),
    };
    let record = use_record_edit::<vitni_ui::ResearchNoteDraft>(Category::ResearchNotes, &human_id, &seed);
    ...
    let on_record_save = use_callback(move |(draft, prov): (vitni_ui::ResearchNoteDraft, ProvenanceDraft)| {
        let services = record_services.clone();
        let edits = draft.edits_against(&record.seed.read());
        let current = current_id.clone();
        let saved = saved_label.clone();
        spawn(async move {
            let effective = apply_record_edits(services, edits, prov, current.clone(), save_research_note_edit).await;
...
/// provenance block.
pub fn research_note_content_tab(loc: &Localizer, record: RecordEditState<vitni_ui::ResearchNoteDraft>) -> Element {
    rsx! {
        ...
        SubjectChooser {
            ...
            onpick: move |(category, selection): (Category, PickerSelection)| {
                onsubmit.call((
                    ResearchNoteEdit::AddSubject {
                        human_id: human_id.clone(),
                        subject: vitni_ui::SubjectRequest { category, human_id: selection.human_id },
                    },
                    prov(),
                ));
            },
