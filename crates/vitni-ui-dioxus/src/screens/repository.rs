...
/// edit, and create.
pub fn repository_record_fields(loc: &Localizer, record: RecordEditState<vitni_ui::RepositoryDraft>) -> Element {
    ...
    let index_of = |repository_type: &Option<vitni_app::RepositoryType>| {
        repository_type
            .as_ref()
...
pub(crate) fn RepositoryDetailPane(human_id: String) -> Element {
    ...
    let seed = match &*data.read_unchecked() {
        Some(ScreenData::Loaded(IntentOutcome::RepositoryDetail(detail))) => {
            vitni_ui::RepositoryDraft::from_detail(detail)
        }
        _ => vitni_ui::RepositoryDraft::new(),
    };
    let record = use_record_edit::<vitni_ui::RepositoryDraft>(Category::Repositories, &human_id, &seed);
    ...
    let on_record_save = use_callback(move |(draft, prov): (vitni_ui::RepositoryDraft, ProvenanceDraft)| {
...
struct RepositoryCallbacks {
    ...
    on_record_save: Callback<(vitni_ui::RepositoryDraft, ProvenanceDraft)>,
...
/// The row callbacks a repository's tabs dispatch through, grouped so the tab dispatcher stays under
/// the argument limit.
#[derive(Clone, Copy)]
#[expect(
    clippy::struct_field_names,
...
fn repository_tab_content(
    ...
    record: RecordEditState<vitni_ui::RepositoryDraft>,
    callbacks: RepositoryTabCallbacks,
) -> Element {
    let loc = state.data_loc();
    let RepositoryTabCallbacks {
        on_retract,
        on_edit_open,
        on_undo,
        on_tag_remove,
    ...
    let shared = SharedTabCtx {
        forms: Some(FormTabs {
            ...
            media: None,
            notes: Some(NotesArm {
                form: RepositoryEditForm::Note,
                rows: &detail.notes,
                on_detach: on_retract,
            }),
            tags: Some(TagsArm {
                form: RepositoryEditForm::Tag,
                rows: &detail.tags,
    ...
    match tab.id {
        ...
        _ => shared_tab(loc, tab, &shared).unwrap_or_else(|| repository_overview(loc, detail, record)),
    }
}
