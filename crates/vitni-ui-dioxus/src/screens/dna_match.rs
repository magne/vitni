...
pub fn DnaMatchCreateRecord(draft_id: DraftId) -> Element {
    ...
    let record = use_record_create::<vitni_ui::DnaMatchDraft>(Category::DnaMatches, draft_id);
    ...
    // (required) test ids.
    let test_a_state = use_signal(vitni_ui::PickerState::default);
    let test_b_state = use_signal(vitni_ui::PickerState::default);
...
pub fn dna_match_create_fields(
    loc: &Localizer,
    mut draft: Signal<vitni_ui::DnaMatchDraft>,
    test_a: &RecordPicker,
    test_b: &RecordPicker,
) -> Element {
...
pub(crate) fn DnaMatchDetailPane(human_id: String) -> Element {
    ...
    let seed = match &*data.read_unchecked() {
        Some(ScreenData::Loaded(IntentOutcome::DnaMatchDetail(detail))) => vitni_ui::DnaMatchDraft::from_detail(detail),
        _ => vitni_ui::DnaMatchDraft::new(),
    };
    let record = use_record_edit::<vitni_ui::DnaMatchDraft>(Category::DnaMatches, &human_id, &seed);
    ...
    let on_record_save = use_callback(move |(draft, prov): (vitni_ui::DnaMatchDraft, ProvenanceDraft)| {
...
struct DnaMatchCallbacks {
    ...
    on_record_save: Callback<(vitni_ui::DnaMatchDraft, ProvenanceDraft)>,
