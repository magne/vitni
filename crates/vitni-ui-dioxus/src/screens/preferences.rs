...
//! Preferences talks straight to `vitni-app` config read/write use-cases
//! (`crate::services::{load_preferences, save_*, make_default_workspace, register_workspace}` and
//! `crate::app::open_workspace`) — there is no `vitni_ui::dispatch`/`Intent` involved, since
...
/// The Preferences screen: loads its data from `vitni-app` config/manifest reads (never opens
...
pub fn PreferencesScreen() -> Element {
    ...
    let onthemechange = move |mode: ThemeMode| {
        ...
        if let Err(error) = vitni_app::save_theme_mode(&theme_services.dir, mode) {
...
/// currently-effective chord (an accepted override, else the default) — no [`vitni_ui::Localizer`]
...
fn surety_layer_label(chrome: &Chrome, layer: LayerKind) -> String {
    match layer {
        ...
        LayerKind::SharedDefault => chrome.prefs_layer_shared("~/.config/vitni/config.toml"),
    ...
    rsx! {
        ...
        Card { title: chrome.prefs_defaults_title(),
            ...
            div { class: "stack", style: "margin-top:8px",
                ...
                {layer_row(chrome, &chrome.prefs_layer_shared("~/.config/vitni/config.toml"), data.layers.theme.winner == LayerKind::SharedDefault)}
...
/// The "Maintenance" card (issue #192): the CLI's `vitni rebuild` GUI counterpart
/// (`Workspace::rebuild_projections`, an ADR 0010 maintenance op). A workspace-functionality op, not
/// an aggregate, so — like the Workspaces card above — it talks straight to `vitni-app` and acts
...
mod tests {
    ...
    use vitni_app::SuretyLabelOverrides;
