...
use vitni_plugin_host::Capability;
...
/// genuinely off each bundle ([`vitni_plugin_host::PluginHost::discover_bundle`]); the effective
...
fn PluginGrantCard(row: PluginRow, onsaved: Callback<()>) -> Element {
    ...
    let vm = vitni_ui::plugin_grant_vm(loc, &row.id, row.trust, &declared, row.approved.as_ref());
...
fn grant_switch(
    loc: &Localizer,
    capability: &vitni_ui::CapabilityGrantVm,
    ...
    rsx! {
        div { class: "card", style: "margin-top:16px",
            ...
            {match &*store.read_unchecked() {
                ...
                Some(Err(message)) => rsx! { p { class: "empty", "{message}" } },
                Some(Ok(config)) => {
                    let vm = vitni_ui::trust_store_vm(config);
...
fn pinned_publisher_row(loc: &Localizer, pin: &vitni_ui::PinnedPublisherVm, on_unpin: Callback<String>) -> Element {
