...
//! [`HelpDoc`](vitni_ui::HelpDoc) vocabulary; this module is the block→RSX interpreter.
//!
//! Block prose resolves through the `vitni-ui` [`Localizer`] catalogue, which owns the help
...
/// `vitni-ui` [`Localizer`] (it owns the help catalogue); the index list chrome comes from the
/// renderer [`Chrome`].
#[component]
...
/// text resolves through the `vitni-ui` [`Localizer`] (which owns the help catalogue).
