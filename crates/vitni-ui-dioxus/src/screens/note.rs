...
pub fn note_record_fields(loc: &Localizer, record: RecordEditState<vitni_ui::NoteDraft>) -> Element {
...
pub(crate) fn NoteDetailPane(human_id: String) -> Element {
    ...
    let seed = match &*data.read_unchecked() {
        Some(ScreenData::Loaded(IntentOutcome::NoteDetail(detail))) => vitni_ui::NoteDraft::from_detail(detail),
        _ => vitni_ui::NoteDraft::new(),
    };
    let record = use_record_edit::<vitni_ui::NoteDraft>(Category::Notes, &human_id, &seed);
    ...
    let on_record_save = use_callback(move |(draft, prov): (vitni_ui::NoteDraft, ProvenanceDraft)| {
        ...
        spawn(async move {
            ...
            finish_record_save(effective, Category::Notes, &current, record_nav, reload, &saved);
        });
    });
...
struct NoteCallbacks {
    ...
    on_record_save: Callback<(vitni_ui::NoteDraft, ProvenanceDraft)>,
