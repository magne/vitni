...
//! application errors) come from [`vitni_ui::Localizer`]; this catalogue is only the GUI's chrome.
...
impl Chrome {
    ...
    pub fn for_workspace(workspace_dir: &Path, config_ui_language: Option<&LanguageIdentifier>) -> Self {
        let requested =
            vitni_app::requested_languages_for(config_ui_language, &DesktopLanguageRequester::requested_languages());
    ...
    pub fn with_languages(workspace_dir: Option<&Path>, requested: &[LanguageIdentifier]) -> Self {
        ...
        vitni_i18n::init(&loader, workspace_dir, shared.as_deref(), requested, Box::new(Embedded));
        Self { loader }
    }
    ...
    #[must_use]
    pub fn theme_mode_label(&self, mode: vitni_app::ThemeMode) -> String {
        match mode {
            vitni_app::ThemeMode::System => fl!(self.loader, "theme-mode-system"),
            vitni_app::ThemeMode::Light => fl!(self.loader, "theme-mode-light"),
            vitni_app::ThemeMode::Dark => fl!(self.loader, "theme-mode-dark"),
        }
    }
    ...
    #[must_use]
    pub fn theme_mode_status(&self, mode: vitni_app::ThemeMode, resolved_dark: bool) -> String {
        match mode {
            vitni_app::ThemeMode::System => {
            ...
            vitni_app::ThemeMode::Light => fl!(self.loader, "status-theme-light"),
            vitni_app::ThemeMode::Dark => fl!(self.loader, "status-theme-dark"),
        }
    }

    /// The accessible name for the theme-cycle control, naming the current mode.
    #[must_use]
    pub fn aria_theme_cycle(&self, mode: vitni_app::ThemeMode) -> String {
    ...
    #[must_use]
    pub fn geography_zoom_in(&self) -> String {
        fl!(self.loader, "geography-zoom-in")
    }

    /// The map's zoom-out button (see [`Self::geography_zoom_in`]).
    #[must_use]
    pub fn geography_zoom_out(&self) -> String {
        fl!(self.loader, "geography-zoom-out")
    }

    /// The scale bar's metre unit.
    #[must_use]
    pub fn geography_scale_meters(&self) -> String {
        fl!(self.loader, "geography-scale-meters")
    }

    /// The scale bar's kilometre unit.
    #[must_use]
    pub fn geography_scale_kilometers(&self) -> String {
        fl!(self.loader, "geography-scale-kilometers")
    }
    ...
    /// A plugin's role, resolved from [`vitni_plugin_host::PluginRole`] to display text.
    #[must_use]
    pub fn plugin_role_label(&self, role: vitni_plugin_host::PluginRole) -> String {
        match role {
            vitni_plugin_host::PluginRole::BulkImport => fl!(self.loader, "plugin-role-bulk-import"),
            vitni_plugin_host::PluginRole::BulkExport => fl!(self.loader, "plugin-role-bulk-export"),
            vitni_plugin_host::PluginRole::UiPanel => fl!(self.loader, "plugin-role-ui-panel"),
            vitni_plugin_host::PluginRole::AssistedImport => fl!(self.loader, "plugin-role-assisted-import"),
            vitni_plugin_host::PluginRole::TestFixture => fl!(self.loader, "plugin-role-test-fixture"),
            vitni_plugin_host::PluginRole::Unknown => fl!(self.loader, "plugin-role-unknown"),
        }
    }

    /// A capability, resolved from [`vitni_plugin_host::Capability`] to display text (the badge
    /// label that makes the badge's colour redundant).
    #[must_use]
    pub fn plugin_capability_label(&self, capability: vitni_plugin_host::Capability) -> String {
        match capability {
            vitni_plugin_host::Capability::Log => fl!(self.loader, "plugin-cap-log"),
            vitni_plugin_host::Capability::Query => fl!(self.loader, "plugin-cap-query"),
            vitni_plugin_host::Capability::Commands => fl!(self.loader, "plugin-cap-commands"),
            vitni_plugin_host::Capability::Progress => fl!(self.loader, "plugin-cap-progress"),
            vitni_plugin_host::Capability::ImportSource => fl!(self.loader, "plugin-cap-import-source"),
            vitni_plugin_host::Capability::ExportSink => fl!(self.loader, "plugin-cap-export-sink"),
            vitni_plugin_host::Capability::Net => fl!(self.loader, "plugin-cap-net"),
            vitni_plugin_host::Capability::MediaStore => fl!(self.loader, "plugin-cap-media-store"),
            vitni_plugin_host::Capability::Ai => fl!(self.loader, "plugin-cap-ai"),
            vitni_plugin_host::Capability::Present => fl!(self.loader, "plugin-cap-present"),
        }
    }
...
mod tests {
    ...
    fn resolves_chrome_strings() {
        ...
        assert_eq!(en.theme_mode_label(vitni_app::ThemeMode::System), "System");
        assert_eq!(en.search_clear(), "Clear search");
        assert_eq!(
            en.theme_mode_status(vitni_app::ThemeMode::System, true),
            "system (dark)"
        );
        assert_eq!(en.theme_mode_status(vitni_app::ThemeMode::Light, true), "light");
        assert_eq!(en.theme_mode_status(vitni_app::ThemeMode::Dark, false), "dark");
        let no = Chrome::with_languages(None, &["no".parse().expect("tag")]);
        assert_eq!(no.nav_people(), "Personer");
        assert_eq!(no.theme_mode_label(vitni_app::ThemeMode::Light), "Lyst");
        assert_eq!(
            no.theme_mode_status(vitni_app::ThemeMode::System, false),
            "system (lyst)"
        );
    }
