...
#[cfg(feature = "desktop")]
fn current_geometry(window: &dioxus::desktop::DesktopContext) -> Option<vitni_app::WindowGeometry> {
    let scale = window.scale_factor();
    let position = window.outer_position().ok()?.to_logical::<i32>(scale);
    let size = window.inner_size().to_logical::<u32>(scale);
    Some(vitni_app::WindowGeometry {
        x: position.x,
        y: position.y,
        width: size.width,
        height: size.height,
...
#[cfg(feature = "desktop")]
fn restore_position(window: &dioxus::desktop::DesktopContext, geometry: vitni_app::WindowGeometry) {
...
#[cfg(feature = "desktop")]
fn persist(dir: &std::path::Path, geometry: vitni_app::WindowGeometry) {
    use vitni_app::ConfigStore as _;
    if let Err(error) = vitni_app::FileConfigStore::for_workspace(dir.to_path_buf()).store_window(geometry) {
