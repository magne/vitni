...
//! listbox. The decision logic is the framework-free [`vitni_ui::palette`] module; this component
...
pub fn CommandPalette() -> Element {
    ...
    let palette_chord = crate::shell::resolved_shortcuts_from_context()
        .into_iter()
        .find(|entry| entry.action == vitni_ui::ShortcutAction::CommandPalette)
...
/// Turns the pure [`vitni_ui::PaletteGroup`]s into rendered views with localized headings, flat
/// option indices (for `aria-activedescendant`), icons, and kind badges.
fn group_views(chrome: &ChromeCtx, groups: &[vitni_ui::PaletteGroup]) -> Vec<GroupView> {
