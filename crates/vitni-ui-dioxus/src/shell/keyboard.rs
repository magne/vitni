...
use dioxus::prelude::*;
use vitni_ui::{Category, Destination, RecordRef, Shortcut, ShortcutAction, ShortcutGroup};
...
/// live resolved shortcut map (`vitni_ui::resolved_shortcuts`, ADR 0030) — every `Global` action
...
pub fn shell_intent(
    ...
    if chord_key == vitni_ui::Key::Question {
...
/// happens — [`vitni_ui::resolved_shortcuts`] always returns every action, at its default chord
/// if no override was accepted).
fn resolved_modifier(resolved: &[Shortcut], action: ShortcutAction) -> Option<vitni_ui::Modifier> {
    resolved
        .iter()
...
/// The event's modifier as a [`vitni_ui::Modifier`]: `command` is the platform primary modifier
/// (resolved by the caller), `shift`/`alt` read straight off the `dioxus` event.
fn event_modifier(modifiers: Modifiers, primary: bool) -> vitni_ui::Modifier {
    vitni_ui::Modifier {
        command: primary,
...
fn shortcut_key(key: &Key) -> Option<vitni_ui::Key> {
    ...
    if character == "?" {
        return Some(vitni_ui::Key::Question);
    }
    ...
    only.is_ascii_alphabetic()
        .then(|| vitni_ui::Key::Char(only.to_ascii_lowercase()))
}
...
mod tests {
    ...
    use vitni_ui::{ShortcutAction, resolved_shortcuts, shortcuts};
    ...
    fn defaults() -> Vec<vitni_ui::Shortcut> {
        shortcuts()
    }
