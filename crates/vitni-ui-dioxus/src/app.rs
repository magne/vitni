...
use vitni_plugin_host::PluginHost;
...
/// (`~/.config/vitni/skin.css`). Read at startup and injected after the embedded base so it
...
/// `VITNI_WORKSPACE` and the configured default. Held in a process-global signal so it survives
...
fn build_state() -> Result<AppState, String> {
    ...
    // Precedence: the in-memory Open override > VITNI_WORKSPACE > the configured default.
