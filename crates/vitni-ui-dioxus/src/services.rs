...
//! [`vitni_ui::dispatch`]; the plugin form is fetched by running the `ui-panel` component and
//! parsing its JSON with [`vitni_ui::parse`].
...
impl Services {
    async fn open(&self) -> Result<Workspace, vitni_app::AppError> {
    ...
    /// `ui_language` < `VITNI_LANGUAGE`. `DesktopLanguageRequester` stays in the renderer so the
    /// app layer is `i18n_embed`-free.
    #[must_use]
    pub fn requested_languages(&self) -> Vec<LanguageIdentifier> {
        vitni_app::requested_languages_for(
    ...
    fn plugin_layers(&self) -> Vec<PathBuf> {
        ...
        vitni_app::plugin_layers(Some(&self.dir), shared.as_deref(), &self.plugins_dir)
    }

    /// Resolves plugin `id` to its bundle directory across the layers (ADR 0014 §4).
    fn plugin_bundle(&self, id: &str) -> Option<PathBuf> {
        vitni_app::resolve_bundle(&self.plugin_layers(), id)
    }
    ...
    fn trust_roots(&self) -> Result<TrustRoots, String> {
        ...
        let pins = vitni_app::resolve_trust_pins(&trust).map_err(|error| chrome.plugin_error(&error.to_string()))?;
...
pub async fn load_screen(services: Services, intent: Intent) -> ScreenData {
    ...
    match vitni_ui::dispatch(&workspace, &loc, &intent).await {
...
pub async fn resolve_record_name(services: Services, category: Category, human_id: String) -> Option<String> {
    ...
    match vitni_ui::resolve_record_name(&workspace, &loc, category, &human_id).await {
        Ok(name) => name,
        Err(error) => {
...
pub async fn commit_family_change_set(
    ...
    let workspace = services.open().await.map_err(|error| loc.error(&error))?;
    let session = Session::new(services.config.operator_agent());
    vitni_ui::dispatch_family_change_set(&workspace, &session, &request, &prov)
        .await
        .map_err(|error| loc.error(&error))
}
...
pub async fn commit_source_change_set(
    ...
    let workspace = services.open().await.map_err(|error| loc.error(&error))?;
    let session = Session::new(services.config.operator_agent());
    vitni_ui::dispatch_source_change_set(&workspace, &session, &request, &prov)
        .await
        .map_err(|error| loc.error(&error))
}
...
pub async fn commit_media_change_set(
    ...
    let workspace = services.open().await.map_err(|error| loc.error(&error))?;
    let session = Session::new(services.config.operator_agent());
    vitni_ui::dispatch_media_change_set(&workspace, &session, &request, &prov)
        .await
        .map_err(|error| loc.error(&error))
}
...
pub async fn commit_research_note_change_set(
    ...
    let workspace = services.open().await.map_err(|error| loc.error(&error))?;
    let session = Session::new(services.config.operator_agent());
    vitni_ui::dispatch_research_note_change_set(&workspace, &session, &request, &prov)
        .await
        .map_err(|error| loc.error(&error))
}
...
pub async fn commit_tag_change_set(
    ...
    let workspace = services.open().await.map_err(|error| loc.error(&error))?;
    let session = Session::new(services.config.operator_agent());
    vitni_ui::dispatch_tag_change_set(&workspace, &session, &request, &prov)
        .await
        .map_err(|error| loc.error(&error))
}
...
pub async fn commit_dna_match_change_set(
    ...
    let workspace = services.open().await.map_err(|error| loc.error(&error))?;
    let session = Session::new(services.config.operator_agent());
    vitni_ui::dispatch_dna_match_change_set(&workspace, &session, &request, &prov)
        .await
        .map_err(|error| loc.error(&error))
}
...
/// (see [`vitni_plugin_host::PluginInfo`]) joined with its persisted enabled/disabled state.
...
pub struct PluginRow {
    ...
    pub role: PluginRole,
    /// The `vitni:host-api` version this component was compiled against.
...
/// `vitni-ui` view-models stay free of plugin-host types (ADR 0014 §3).
...
pub async fn discover_plugins(services: Services) -> Result<Vec<PluginRow>, String> {
    ...
    let bundles = vitni_app::resolve_bundles(&services.plugin_layers());
    let prefs = vitni_app::read_plugin_preferences(&services.dir);
...
pub async fn set_plugin_grants(services: Services, id: String, approved: BTreeSet<String>) -> Result<(), String> {
    let loc = services.localizer();
    vitni_ui::approve_plugin_grants(&services.dir, &id, &approved).map_err(|error| loc.error(&error))
}
...
pub async fn pin_publisher(services: Services, publisher: String, public_key_hex: String) -> Result<(), String> {
    let chrome = services.chrome();
    let path = config::config_path().map_err(|error| chrome.plugin_error(&error.to_string()))?;
    vitni_ui::pin_publisher(&path, &publisher, &public_key_hex).map_err(|error| chrome.plugin_error(&error.to_string()))
}
...
pub async fn unpin_publisher(services: Services, publisher: String) -> Result<(), String> {
    let chrome = services.chrome();
    let path = config::config_path().map_err(|error| chrome.plugin_error(&error.to_string()))?;
    vitni_ui::unpin_publisher(&path, &publisher).map_err(|error| chrome.plugin_error(&error.to_string()))
}
...
pub async fn load_plugin_panel(services: Services) -> Result<Panel, String> {
    ...
    let (json, _workspace) = services
        ...
        .await
        .map_err(|error| chrome.plugin_error(&error.to_string()))?;
    let panel = vitni_ui::parse(&json).map_err(|error| chrome.plugin_error(&error.to_string()))?;
    let requested = services.requested_languages();
    Ok(vitni_ui::resolve_panel(
        &panel,
...
pub async fn submit_plugin_panel(services: Services, action: String, values: String) -> Result<SubmitResult, String> {
    ...
    let (json, _workspace) = services
        ...
        .await
        .map_err(|error| chrome.plugin_error(&error.to_string()))?;
    let result = vitni_ui::parse_submit_result(&json).map_err(|error| chrome.plugin_error(&error.to_string()))?;
    let requested = services.requested_languages();
    Ok(vitni_ui::resolve_submit_result(
        &result,
        &services.plugin_catalogue_dir(UI_PANEL_DOMAIN),
        UI_PANEL_DOMAIN,
        &requested,
...
/// [`Confidence::Low`](vitni_app::Confidence::Low) provenance template (ADR 0017 §7).
...
/// [`vitni_plugin_host::PluginHost::run_assisted_import`] with the channel-backed `presenter`.
...
/// or a localized error. Mirrors the CLI's `vitni export` (ADR 0013): resolve the bundle, take the
...
/// invocation runs under a Software operator with a deny-all network policy, as the CLI's `vitni
/// import` does.
...
/// registered one. Registering calls `vitni_app::register_workspace` directly (not this module's
...
async fn open_import_target(services: &Services, target: &ImportTargetChoice) -> Result<Workspace, String> {
    ...
    match target {
        ...
        ImportTargetChoice::New {
            ...
            let summary =
                vitni_app::register_workspace(&config_path, name, directory.as_deref(), database_url.as_deref())
                    .await
                    .map_err(|error| loc.error(&error))?;
            Workspace::open(
                &summary.path,
...
/// erroring (mirrors [`vitni_app::read_preference_layers`]).
#[must_use]
...
/// [`vitni_app::register_workspace`]. The caller triggers the application-state restart on
/// success.
...
/// on failure. The Preferences Maintenance card's counterpart to the CLI's `vitni rebuild`
/// (`Workspace::rebuild_projections`, an ADR 0010 maintenance op) — the tool for recovering after a
/// `vitni-db` schema change.
