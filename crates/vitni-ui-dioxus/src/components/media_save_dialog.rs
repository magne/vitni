...
//! target come from the framework-free [`MediaSaveDraft`](vitni_ui::MediaSaveDraft); the host's
//! `media-store` writes the file under the media root and enforces path safety.

use dioxus::prelude::*;
use vitni_ui::MediaSaveDraft;
...
/// [`target_rel_path`](vitni_ui::MediaSaveDraft::target_rel_path) is previewed live) and receives
...
pub fn MediaSaveDialog(
    ...
    // (`vitni_core::media_path`) so the preview cannot drift from what lands in the record.
    let stored_path = vitni_app::workspace_media_path(&preview);
