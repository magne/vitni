...
fn help_open_with_rebound_quit() -> Element {
    ...
    use_context_provider(|| {
        ...
        ShortcutsCtx(Signal::new(vitni_app::ShortcutConfig { bindings }))
    });
    let mut nav = use_context_provider(NavState::new);
    use_hook(|| nav.overlay.set(Overlay::Help));
    rsx! {
...
fn help_overlay_renders_the_shortcut_map() {
    ...
    assert!(
        html.contains("<kbd>Ctrl</kbd><kbd>←</kbd>"),
        "the history-back chord row (issue #313):\n{html}"
    );
    assert!(
        html.contains("<kbd>Ctrl</kbd><kbd>→</kbd>"),
        "the history-forward chord row (issue #313):\n{html}"
    );
