...
fn config_with_one_workspace() -> Config {
    ...
    Config {
        ...
        defaults: vitni_app::AppDefaults::default(),
        workspace_defaults: WorkspaceDefaults::default(),
        ai: vitni_app::AiConfig::default(),
        map: vitni_app::MapConfig::default(),
        plugin_trust: vitni_app::PluginTrustConfig::default(),
        shortcuts: vitni_app::ShortcutConfig::default(),
    }
}
...
fn render_prefs(
    ...
    shortcuts: vitni_app::ShortcutConfig,
    status: Option<String>,
...
fn shortcut_bindings_seed(config: &vitni_app::ShortcutConfig) -> BTreeMap<String, String> {
    let (resolved, _errors) = vitni_ui::resolved_shortcuts(&config.bindings);
    resolved
        .into_iter()
        .filter(|entry| entry.group == vitni_ui::ShortcutGroup::Global)
...
fn one_workspace_with_shortcut_override_and_rejection() -> Element {
    ...
    render_prefs(
        ...
        resolved_locale(DateFormat::Long, NumberFormat::SpaceComma),
        SuretyLabelOverrides::default(),
        vitni_app::ShortcutConfig { bindings },
        None,
        workspaces,
        open,
        false,
...
fn defaults_card_marks_the_workspace_layer_as_the_winner_when_pinned() {
    ...
    assert!(
        html.contains("~/.config/vitni/config.toml"),
...
fn defaults_card_marks_the_shared_default_as_the_winner_when_unpinned() {
    ...
    let shared_row_start = html.find("~/.config/vitni/config.toml").expect("shared row");
...
/// point a new workspace at Postgres, since `vitni-app`'s postgres backend isn't compiled in
/// either (Cargo.toml's `postgres = ["vitni-app/postgres"]`).
...
/// Arming the confirm signal renders the [`Modal`](vitni_ui_dioxus::components::Modal) with its
