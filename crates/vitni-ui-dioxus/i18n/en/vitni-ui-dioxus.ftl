# Chrome strings for the Dioxus GUI (ADR 0003, ADR 0008 §3). Data strings come from vitni-ui.
...
# vitni-ui catalogue (resolved via Localizer) — these are the renderer's
...
# sentence live in the vitni-ui catalogue (data, resolved via Localizer) — these are the
...
geography-drawing-on = Drawing on { $place }
geography-draw-target-none = No place selected
...
geography-zoom-aria = Zoom level { $level }
geography-zoom-in = Zoom in
geography-zoom-out = Zoom out
...
# labels above) live here, not in the vitni-ui data catalogue — they name UI settings, not
...
prefs-maintenance-intro = Rebuild every projection from the event log. Projections are derived state, not the source of truth, so this never loses data — use it after a vitni-db schema change, or if a projection looks out of sync with the log.
...
# Bulk-export wizard (ADR 0013): the GUI counterpart of `vitni export`.
...
# Bulk-import wizard (issue #191): the GUI counterpart of `vitni import`.
