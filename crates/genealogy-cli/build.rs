// build.rs — generates `genealogy-cli.ftl` for each locale by concatenating all per-aggregate
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for entry in fs::read_dir(i18n_dir)? {
        ...
        // Exclude the generated genealogy-cli.ftl itself to avoid reading our own output.
        let mut ftl_files: Vec<_> = fs::read_dir(&locale_dir)?
            ...
            .filter(|p| {
                p.file_name()
                    .and_then(|n| n.to_str())
                    .is_some_and(|n| n != "genealogy-cli.ftl")
            })
            .collect();
        ftl_files.sort();
        ...
        fs::write(locale_dir.join("genealogy-cli.ftl"), &combined)?;
