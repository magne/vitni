//! Framework-free boundary guard (ADR 0008 §§2,4): `genealogy-ui` holds all presentation logic and
//! **no framework types**. Dependencies flow one way — `genealogy-app → genealogy-ui →
//! genealogy-ui-<framework>` — so no `dioxus::` (or future `slint::`) type may appear at or below
...
//!
//! 1. **Dependency-tree guard** — `genealogy-ui`'s transitive dependency closure (resolved offline
...
/// The framework crates that must never enter `genealogy-ui`. A package is a match if its name is
...
fn matcher_flags_a_crate_and_its_family_but_not_a_lookalike() {
    ...
    assert!(!framework_matches("genealogy-ui", BANNED_FRAMEWORKS));
}

#[test]
fn graph_walk_detects_a_framework_crate_in_the_closure() {
    let mut graph = BTreeMap::new();
    graph.insert("genealogy-ui".to_string(), vec!["genealogy-app".to_string()]);
    graph.insert("genealogy-app".to_string(), vec!["dioxus".to_string()]);
    graph.insert("dioxus".to_string(), vec!["dioxus-core".to_string()]);
    graph.insert("dioxus-core".to_string(), Vec::new());

    let found = framework_crates_in_graph(&graph, "genealogy-ui", BANNED_FRAMEWORKS);
...
fn graph_walk_is_empty_for_a_clean_closure() {
    let mut graph = BTreeMap::new();
    graph.insert("genealogy-ui".to_string(), vec!["genealogy-app".to_string()]);
    graph.insert("genealogy-app".to_string(), vec!["serde".to_string()]);
    graph.insert("serde".to_string(), Vec::new());

    assert!(framework_crates_in_graph(&graph, "genealogy-ui", BANNED_FRAMEWORKS).is_empty());
}
...
fn source_scan_ignores_a_framework_reference_in_a_comment() {
    let clean = "//! No `dioxus::` type appears here (ADR 0008).\nuse genealogy_app::Workspace;\n";
    assert!(framework_tokens_in_source(clean, BANNED_FRAMEWORKS).is_empty());
}

#[test]
fn genealogy_ui_dependency_closure_is_framework_free() {
    let meta = MetadataCommand::new().exec().expect("run cargo metadata");
    let found = framework_crates_in_closure(&meta, "genealogy-ui", BANNED_FRAMEWORKS);
    assert!(
        found.is_empty(),
        "genealogy-ui pulls banned framework crates: {found:?}"
    );
}

#[test]
fn genealogy_ui_sources_reference_no_framework() {
    ...
    assert!(
        offenders.is_empty(),
        "framework references in genealogy-ui/src: {offenders:?}"
    );
}
