...
//!
//! Runs against a real on-disk workspace over `genealogy-app`'s public surface only — the presentation
//! layer never names a `genealogy-core` type, even in tests (ADR 0008).
...
async fn load_detail(ws: &Workspace, loc: &Localizer, human_id: &str) -> Box<genealogy_ui::ResearchNoteDetail> {
...
async fn the_reverse_lookup_rows_reach_all_four_subject_detail_screens() {
    ...
    .await
    .expect("event");
    let family = genealogy_app::create_family(&ws, &session, Provenance::default(), &[])
        .await
        .expect("family");
...
fn assert_reverse_tab(tabs: &[genealogy_ui::DetailTab]) {
    let position = tabs
        .iter()
