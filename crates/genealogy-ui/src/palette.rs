...
/// existing [`NavState`](../../genealogy_ui_dioxus/shell/nav_state) action.
...
/// Find-duplicates, the tool screens (Pedigree/Plugins/Preferences), and Help. The renderer resolves
/// each command's label. Merge is reached through [`PaletteCommand::FindDuplicates`], so it is not an
/// [`PaletteCommand::OpenTool`]; GEDCOM import has no GUI path, so there is no import command.
#[must_use]
