...
use genealogy_app::{
    AiConfig, AiProvider, AppDefaults, Confidence, OperatorConfig, Session, Workspace, WorkspaceDefaults,
};
use genealogy_core::ids::AgentId;
use genealogy_core::provenance::{Agent, AgentKind};
use genealogy_plugin_host::{Capability, Grants, HostPattern, NetPolicy, PluginError, ResourceBudget};
use serde_json::json;
use uuid::Uuid;
...
async fn vision_api_sends_the_openai_request_shape_and_reads_string_content() {
    const KEY_ENV: &str = "GENEALOGY_TEST_AI_KEY_STRING";
...
async fn vision_api_reads_content_parts() {
    const KEY_ENV: &str = "GENEALOGY_TEST_AI_KEY_PARTS";
    let server = MockServer::start().await;
    let base = base_url(&server);
    Mock::given(method("POST"))
        .and(path("/chat/completions"))
...
async fn vision_api_error_status_is_a_backend_error_without_the_key() {
    const KEY_ENV: &str = "GENEALOGY_TEST_AI_KEY_ERROR";
    let server = MockServer::start().await;
    let base = base_url(&server);
    Mock::given(method("POST"))
        .and(path("/chat/completions"))
...
async fn vision_api_missing_env_var_is_invalid_input_naming_the_var() {
    const KEY_ENV: &str = "GENEALOGY_TEST_AI_KEY_ABSENT";
    let server = MockServer::start().await;
    let base = base_url(&server);
