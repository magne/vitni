...
//! is load-bearing: `vitni-app` parses `kind` back with `serde_json`, so any drift between the
...
use vitni_core::assertions::{Asserted, Attributed};
use vitni_core::place_succession::PlaceSuccessionAssertion;
...
/// backends store verbatim and `vitni-app` parses back.
///
/// # Errors
///
