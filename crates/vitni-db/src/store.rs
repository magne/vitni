...
//!
//! [`Store`] is the entire public surface of `vitni-db`: it is opened from a `database_url` and
...
//! `vitni-app` or frontends. When both backends are compiled in, a single binary selects the
...
pub enum CommandError<E: std::error::Error + 'static> {
    /// A domain rule rejected the command (from `vitni-core`).
    #[error(transparent)]
    Rejected(E),
...
impl Store {
    ...
    pub async fn list_research_notes_for_subject(
        &self,
        subject: vitni_core::research_note::subject::SubjectRef,
...
#[cfg(any(feature = "sqlite", feature = "postgres"))]
fn subject_key_value(subject: vitni_core::research_note::subject::SubjectRef) -> (&'static str, String) {
    ...
    ($(($snake:ident, $next:ident, $table_const:ident)),+ $(,)?) => {
        impl Store {
            $(
                ...
                ///
                /// [`DbError`] on a read-model failure.
                pub async fn $next(&self, format: &vitni_core::id_format::IdFormat) -> Result<String, DbError> {
                    #[cfg(any(feature = "sqlite", feature = "postgres"))]
                    {
                        match &self.backend {
                            #[cfg(feature = "sqlite")]
