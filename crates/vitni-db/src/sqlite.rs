...
mod tests {
    ...
    async fn a_person_is_found_by_its_external_id() {
        use vitni_core::text::ExternalId;
    ...
    async fn citation_against_a_missing_source_is_rejected_through_services() {
        ...
        use vitni_core::citation::error::CitationError;
        use vitni_core::ids::{CitationId, SourceId};
    ...
    async fn event_linking_a_missing_place_is_rejected_through_services() {
        ...
        use vitni_core::event::error::EventError;
        use vitni_core::ids::{EventId, PlaceId};
    ...
    async fn source_linking_a_missing_repository_is_rejected_through_services() {
        use vitni_core::enums::SourceMediaType;
        use vitni_core::ids::{RepositoryId, SourceId};
    ...
    async fn rebuild_reproduces_identical_projections() {
        ...
        use vitni_core::event::command::{EventCommand, EventCommandEnvelope};
        use vitni_core::family::command::{FamilyCommand, FamilyCommandEnvelope};
        use vitni_core::ids::{CitationId, EventId, FamilyId, PlaceId, SourceId};
        use vitni_core::place::command::{PlaceCommand, PlaceCommandEnvelope};
        use vitni_core::source::command::{SourceCommand, SourceCommandEnvelope};
    ...
    async fn repository_projects_and_rebuilds_identically() {
        ...
        use vitni_core::enums::RepositoryType;
        use vitni_core::ids::RepositoryId;
        use vitni_core::repository::command::{RepositoryCommand, RepositoryCommandEnvelope};
    ...
    async fn place_enclosed_by_a_missing_place_is_rejected_through_services() {
        ...
        use vitni_core::place::command::{PlaceCommand, PlaceCommandEnvelope};
        use vitni_core::place::error::PlaceError;
    ...
    async fn dump_place_succession_links(store: &SqliteStore) -> Vec<(String, String)> {
        sqlx::query("SELECT from_place_id, to_place_id FROM place_succession_link ORDER BY from_place_id, to_place_id")
            .fetch_all(&store.pool)
            .await
            .unwrap()
            .iter()
            .map(|row| (row.get("from_place_id"), row.get("to_place_id")))
            .collect()
    }

    #[tokio::test]
    async fn place_succession_index_is_symmetric_and_rebuilds_identically() {
        use vitni_core::enums::{PlaceType, SuccessionKind};
        use vitni_core::ids::PlaceId;
        use vitni_core::place::command::{PlaceCommand, PlaceCommandEnvelope};
        ...
        // Aker + Kristiania merged into Oslo (1948) — the assertion is recorded once, on Aker's own
        // stream (the anchor), naming both endpoint lists (ADR 0026 §3).
        store
            .execute_place(
                &aker.to_string(),
                PlaceCommandEnvelope {
        ...
        // Symmetric and navigable from every endpoint, not just the anchor (ADR 0026 §4).
        let oslo_predecessors = store.place_predecessors(&oslo.to_string()).await.unwrap();
        let mut predecessor_ids: Vec<_> = oslo_predecessors.iter().map(|r| r.place_id.clone()).collect();
        predecessor_ids.sort();
        let mut expected = vec![aker.to_string(), kristiania.to_string()];
        expected.sort();
        assert_eq!(predecessor_ids, expected);
        assert_eq!(
            oslo_predecessors[0].kind, "\"Merged\"",
            "kind is JSON-serialized, like date_json"
        );

        let aker_successors = store.place_successors(&aker.to_string()).await.unwrap();
        assert_eq!(aker_successors.len(), 1);
        assert_eq!(aker_successors[0].place_id, oslo.to_string());

        let kristiania_successors = store.place_successors(&kristiania.to_string()).await.unwrap();
        assert_eq!(kristiania_successors[0].place_id, oslo.to_string());

        let before = dump_place_succession_links(&store).await;
        store.rebuild_projections().await.unwrap();
    ...
    async fn dna_test_for_a_missing_person_is_rejected_through_services() {
        ...
        use vitni_core::dna_test::error::DnaTestError;
        use vitni_core::ids::{DnaTestId, PersonId};
    ...
    async fn dna_test_for_a_present_person_projects_and_rebuilds_identically() {
        use vitni_core::dna::{DnaProvider, DnaTestType};
        use vitni_core::dna_test::command::{DnaTestCommand, DnaTestCommandEnvelope};
        use vitni_core::enums::EvidenceLevel;
        use vitni_core::ids::{DnaTestId, PersonId};
    ...
    async fn dna_match_between_present_tests_projects_and_rebuilds_with_fixed_point_cm() {
        use vitni_core::dna::{Centimorgans, DnaProvider};
        use vitni_core::dna_match::command::{DnaMatchCommand, DnaMatchCommandEnvelope};
        use vitni_core::dna_match::error::DnaMatchError;
        use vitni_core::dna_test::command::{DnaTestCommand, DnaTestCommandEnvelope};
        use vitni_core::enums::EvidenceLevel;
        use vitni_core::ids::{DnaMatchId, DnaTestId, PersonId};
    ...
    async fn research_note_against_a_missing_subject_is_unknown_subject() {
        ...
        use vitni_core::research_note::error::ResearchNoteError;
        use vitni_core::research_note::subject::SubjectRef;

        let (store, _dir) = store().await;
        let research_note_id = vitni_core::ids::ResearchNoteId::from_uuid(Uuid::from_u128(1));
    ...
    async fn research_note_against_two_present_subjects_projects_and_is_found_by_either() {
        ...
        let research_note_id = vitni_core::ids::ResearchNoteId::from_uuid(Uuid::from_u128(3));
        store
            .execute_research_note(
                &research_note_id.to_string(),
                ResearchNoteCommandEnvelope {
