...
| **29** | Correction model — per-row Edit(=supersede)/Retract + Person tagging | `genealogy-ui` intents, `genealogy-ui-dioxus` table rows | Every collection row gets **Edit** (opens the row's form pre-filled; save = PR24 supersede by `AssertionId`) and **Retract** (undo path + PR24 rationale) per `record-editing.html` §8 — today the only correction is History-undo of an entry. Add the missing `PersonEdit::Tag{remove}` variant + tag-chip × on Person (done on the other 10). Detach citation/media/note = retract of the attach assertion — no new app verbs. |
