...
| Artifact                                   | Contents                                                                                     |
| ------------------------------------------ | -------------------------------------------------------------------------------------------- |
| `genealogy-<ver>-linux-<arch>.tar.gz`      | `genealogy` (CLI) + `genealogy-gui` (GUI) + `plugins/` (signed fleet) + README + `.desktop` + icon |
| `genealogy_<ver>_<arch>.deb` (CLI)         | `/usr/bin/genealogy` + fleet in `/usr/lib/genealogy/plugins`                                  |
| `genealogy-gui_<ver>_<arch>.deb` (GUI)     | `/usr/bin/genealogy-gui` + `.desktop` + icon + fleet in `/usr/lib/genealogy/plugins`         |
| `Genealogy-x86_64.AppImage`                | self-contained GUI + bundled fleet (its `AppRun` points `GENEALOGY_PLUGIN_DIR` at the fleet) |
...
`$GENEALOGY_PLUGIN_DIR` (when set) else the dev source tree; a packaged install points that variable
...
- **Private signing key** — the ed25519 seed that signs the sanctioned fleet. **Never in the repo.**
  Held as the CI secret `GENEALOGY_PLUGIN_SIGNING_KEY` (64 hex chars = a 32-byte seed) and used by
  `cargo xtask build-plugins` / `cargo xtask package` at release time. When unset, both fall back to
  the deterministic **dev key** (`genealogy_plugin_host::signing::DEV_SEED`) — fine for local
  iteration, never a release.
- **Public trust root** — the matching ed25519 public key (64 hex). Provided to the release build as
  `GENEALOGY_PROJECT_PUBLIC_KEY`, read via `option_env!` at **compile time** and embedded in every
  ...
  no `GENEALOGY_PROJECT_PUBLIC_KEY` trusts **no** sanctioned key. The public key is safe to commit.
...
- **Rotation** — embed the current **and** the previous public key(s) in the binary's sanctioned set
  ...
  embedded key). (Today `GENEALOGY_PROJECT_PUBLIC_KEY` carries one key; a rotation set extends this.)
...
2. Ensure the release secrets are configured on the repository: `GENEALOGY_PLUGIN_SIGNING_KEY` and
   `GENEALOGY_PROJECT_PUBLIC_KEY`.
...
- **Tarball** (no external tooling): `cargo xtask package` → `target/dist/genealogy-<ver>-linux-<arch>.tar.gz`.
  Set `GENEALOGY_PLUGIN_SIGNING_KEY` to release-sign; otherwise the dev key is used and every staged
...
- **`.deb`** (needs `cargo install cargo-deb`): run `cargo xtask build-plugins` first (the fleet must
  exist), then `cargo deb -p genealogy-cli` and `cargo deb -p genealogy-ui-dioxus -- --features desktop`
  → `target/debian/*.deb`.
- **AppImage**: needs `appimagetool` (the workflow downloads a SHA-256-pinned `continuous` build).

## Residuals

- **System default embedded path.** The `.deb` installs the fleet to `/usr/lib/genealogy/plugins`, but
  the loader has no built-in default for that path yet — a `.deb`-installed GUI/CLI needs
  `GENEALOGY_PLUGIN_DIR=/usr/lib/genealogy/plugins` (the AppImage sets it via `AppRun`; the tarball
