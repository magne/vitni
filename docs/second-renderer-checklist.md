...
flow one way, `vitni-app → vitni-ui → vitni-ui-<framework>`, and a second framework is
**additive** — a new renderer crate that reuses `vitni-ui` unchanged (ADR 0008 §7). This is the
ordered checklist a `vitni-ui-<framework>` crate follows. `vitni-ui-dioxus` is the worked
example throughout.

The one rule everything else serves: **no framework type appears at or below `vitni-ui`.** The
framework-free guard (`crates/vitni-ui/tests/framework_free.rs`) makes that executable — it fails
if `vitni-ui`'s dependency closure gains a `dioxus`/`slint` crate or a `src` file references a
framework namespace. A new renderer changes nothing in `vitni-ui`, so that guard keeps passing.
...
- The app's own screens are per-framework view code over the **shared view-models** from
  `vitni-ui` — full access to the framework's rich components. Do not render the whole app
  through the plugin vocabulary (ADR 0008 §5); that would rebuild a framework and discard why one
  was chosen.
- UI events dispatch back through `vitni-ui`'s intents/navigation, which call `vitni-app`
  use-cases. The renderer holds no domain rules or coordination.
- Anything the renderer consumes from the app must be re-exported from the `vitni-app` root
  `pub use` block (the app's public surface) — and, when it flows through the presentation layer,
  surfaced by `vitni-ui`. Add those exports before wiring the consumer.
- **Keyboard shortcuts are a lookup against a resolved map, not a hardcoded matcher** (ADR 0030). Load
  the client-scope `[shortcuts]` overrides (`vitni_app::ShortcutConfig`) and call
  `vitni_ui::resolved_shortcuts` to get the one `Vec<Shortcut>` both the dispatcher and the `?`
  help overlay read; translate the framework's own key event into a `vitni_ui::Chord`
  (`Modifier { command, shift, alt }` + `Key`) and look it up — never re-implement the chord matrix
  per renderer (mirror `vitni-ui-dioxus/src/shell/keyboard.rs::shell_intent`). Only
...
- Plugin-contributed screens emit the constrained, serializable UI vocabulary defined in
  `vitni-ui` (`vocabulary.rs`: `Panel` → `Form`/`Table`, field kinds, `Action`s, `SubmitResult`).
  Each renderer maps that vocabulary to native widgets **once**, and every plugin reuses it.
- Mirror the Dioxus interpreter (`vitni-ui-dioxus/src/vocabulary_render.rs`): a top `Panel` view
...
- Resolve only what `vitni-ui` says is a Fluent id — action labels, table column headers — via
...
- Drive the plugin host directly from this crate under a software-operator session
  ...
  (ADR 0011 §5). `vitni-ui` never sees a host type — the renderer hands it the plugin's JSON.
...
- The renderer owns its own `i18n/<lang>/*.ftl` chrome catalogue embedded with `RustEmbed`, layered
  over runtime overrides (workspace > app-dir > embedded), exactly like `vitni-ui-dioxus/i18n`.
- Chrome strings (window/navigation labels, renderer-level errors) resolve through this catalogue;
  data strings (names, field labels, application errors) come from `vitni_ui::Localizer`. Every
...
- Render components to a string in tests instead of opening a GUI, the way the Dioxus renderer does
  with `dioxus_ssr::render` over a `VirtualDom` (see `crates/vitni-ui-dioxus/tests/`). Assert on
...
- Cover the interpreter end to end: run a plugin through the host, parse its JSON with
  `vitni-ui`, render through the interpreter, assert the output (the `interpreter.rs` /
  `plugin_submit.rs` pattern). This needs the plugins built first: `cargo xtask build-plugins`.

## 6. Prove the boundary held

- `vitni-ui` must be untouched. Run the framework-free guard —
  `cargo test -p vitni-ui --test framework_free` — and the full gates
  (`cargo nextest run --workspace --all-features --lib --bins --tests`, clippy, fmt, i18n-check,
  `cargo deny check`). A second renderer that reused `vitni-ui` as-is leaves every one green.
