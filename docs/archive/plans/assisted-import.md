...
Confirmed in `genealogy-core` / `genealogy-app`:

- **Crop rectangle** — `Rect` and `MediaRef.crop` (`genealogy-core/src/text.rs`) already model a
...
### 2. `genealogy-digitalarkivet` pure crate (ADR 0017 §6; research §3)
...
use-cases/DTOs re-exported from `genealogy-app/src/lib.rs` **first** (export-before-consume, incl.
`Rect`/`CropRect`).
...
- **Crop tool:** framework-free math in `genealogy-ui/src/view_model/crop.rs` (`rect_from_drag`,
...
- **Media save dialog:** modal; Category = SelectInput over the fixed convention list ∪ existing
  ...
  `genealogy-ui/src/view_model/media_save.rs`; live relative-path preview. Outputs the relative target
...
and `run_assisted_import(component, invocation, request, presenter, progress)`. `genealogy-ui`:
...
wired in `genealogy-ui-dioxus/src/services.rs`; the `Tool::Import` wizard screen. Cancellation:
...
- `genealogy-plugin-host`: `wit/host.wit` (`net`, `media-store`, `ai`, `present`, `assisted-import`
...
- `genealogy-digitalarkivet` (new crate): `html` parsers + `api` seam; `tests/fixtures/` (verbatim).
- `genealogy-app`: `[ai]` config types in the client scope of `ConfigStore`; `MediaRefInput`; six
...
- `genealogy-ui`: `import_payload.rs`; `view_model/{import,crop,media_save}.rs`; ~45 Fluent keys
  (en+no) `import-*`, `media-viewer-*`, `media-save-*`, `nav-import`.
- `genealogy-ui-dioxus`: asset handler + `resolve_media_path`; `screens/import.rs`; media viewer
  overlay + crop tool + save dialog; gallery thumbnails; `services.rs` Presenter.
- `genealogy-gramps-xml`: `<region>` ↔ `crop` mapping.
...
- `prek.toml`: fixture-fixer exclusion for `crates/genealogy-digitalarkivet/tests/fixtures/`.
...
| 3 | `feat/digitalarkivet-crate` | `genealogy-digitalarkivet` (+`scraper`); verbatim fixtures; prek exclusion | per-fixture parsers; viewer→image chain; URN/ExternalId; classification; malformed-HTML typed errors; wasm32-wasip2 build |
...
| 7 | `feat/host-present-assisted` | WIT 0.19.0 `present` + `assisted-import` world; `Presenter` + `run_assisted_import`; `genealogy-ui` `import_payload.rs` + `ImportSession`; GUI Presenter in `services.rs`; fixture `try-present` | scripted-Presenter (submit/cancel/channel-drop); payload parse incl. malformed; session-machine units |
...
- **Manual GUI run** (`cargo run -p genealogy-ui-dioxus`): Tools → Import; paste a real Digitalarkivet
...
- `prek.toml` currently names the planned `genealogy-import` fixtures path; PR3 updates it to
  `genealogy-digitalarkivet`.
