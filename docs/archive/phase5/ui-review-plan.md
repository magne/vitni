...
- **Person Events tab is thinner than every mockup/table peer:** `events_table`
  (`vitni-ui-dioxus/src/screens/person.rs:1355`) renders Event · Role · Date only — no
...
- **Family screen has no Citations tab:** `family_tabs`
  (`vitni-ui/src/view_model/family.rs:325`) lists
...
- **Event addresses exist app-side, invisible UI-side:** `EventDetail.addresses`
  (`vitni-app/src/event.rs:96`) + `add_event_address` (`:292`) are wired; zero `address`
...
- **Source "Used by N records"** — computed (`SourceReliability.record_count`), never
  rendered.
- **List sort exists in the VM** (`vitni-ui/src/list.rs:62` `RowSort`, applied by
...
count. Crates: `vitni-ui`, `vitni-ui-dioxus`, `vitni-app` (DTO fields), i18n
fragments en+no.
...
Crates: `vitni-ui-dioxus` (bundled CSS + screens + SSR tests).
...
Findings: §6 Repository fax/www. Crates: `vitni-core` (view accessor),
`vitni-app`, `vitni-ui`, `vitni-ui-dioxus`.
...
Findings: §6 Event `Address`. Crates: `vitni-ui`, `vitni-ui-dioxus`
...
Findings: §6 items. Crates: `vitni-ui`, `vitni-ui-dioxus`.
...
Findings: §6 timeline. Crates: `vitni-app` (or ui-only if DTOs suffice),
`vitni-ui`, `vitni-ui-dioxus`.
...
Findings: §6 DNA inference (ui-review U17's substance). Crates: `vitni-core`,
`vitni-app`, `vitni-ui`, `vitni-ui-dioxus`; small ADR or data-model §12 note.
